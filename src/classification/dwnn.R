#######################################################
# Code implemented to model the dataset using DwNN 
#
#######################################################
# Date: January, 2021
#
...
dwnn.classif<-function(train.task, test.task, measure = list(acc), save.model=NULL, max.k=60){
  ...
  for(i in 2:max.k){
    out <- distMat.KernelKnn(distances, TEST_indices = test.task, y = label, k = i, 
                             regression = F, weights_function = 'gaussian', threads = detectCores(),
