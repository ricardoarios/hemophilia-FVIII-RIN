...
cv.class<-function(dataset, seed=123456, cv=10){
  set.seed(seed)
  
  cv.folds<-list()
  
  n.data<-round(nrow(dataset)/cv, 0)
  #random.data<-sample(1:nrow(dataset))
  random.data<-1:nrow(dataset)
  
  for(cv.i in 1:(cv-1)){
    start<-((cv.i-1)*n.data+1)
    cv.folds[[cv.i]]<-random.data[start:(start+n.data-1)] #shuffle our cv.values
  }
  
  cv.folds[[cv]]<-random.data[((cv-1)*n.data+1):length(random.data)]
  invisible(cv.folds)
}
...
cv.bin.strat.class<-function(dataset, seed=123456, cv=10){
  ...
  balance<-table(dataset$Activity)  
  ...
  for(cv.i in 1:(cv-1)){
    tmp<-which(dataset$Activity[actual.inst]==(names(balance)[1]))
    c1<-actual.inst[sample(tmp, n.inst.c0)]
    tmp<-which(dataset$Activity[actual.inst]==(names(balance)[2]))
